...
// -----------------------------------------------------
// NB! columns in azColName are in the same order as listed in SELECT statement.
//     if we SELECT *, coumns are in the order from CREATE TABLE statement
// -----------------------------------------------------
...
void mx_do_login(sqlite3 *db, t_list *cur_client) {
    ...
    memset(sql, 0, 128);
    sprintf(sql,
            "SELECT password, uid, first_name, last_name FROM users WHERE user_name = '%s';", cur_client->auth_req_res->username);
