...
static void init_ok(t_client *client) {
    GtkBuilder *builder;

    builder = gtk_builder_new_from_file("client/templates/window.ui");
    if (!builder) g_critical("Builder getting error!");
    gtk_builder_connect_signals(builder, NULL);

    client->ui->ok_window = GTK_WIDGET(gtk_builder_get_object(builder, "ok_window"));
    if (!client->ui->ok_window) g_critical("Can't get OK window");
...
//         case LOGIN:
//             gtk_widget_hide(client->ui->err_dialog);
//             gtk_widget_show(client->ui->ok_window);
//             return G_SOURCE_CONTINUE;
//         case REGISTRATION:
//             gtk_widget_hide(client->ui->ok_window);
//             gtk_widget_show(client->ui->ok_window);
//             return G_SOURCE_CONTINUE;
//         case CHAT:
//             gtk_widget_hide(client->ui->ok_window);
